  ...
  render() {
    return (
      <div id={'card-'+this.props.word} name='card' className={styles.card + ' ' + styles[this.state.value]} onClick={this.click.bind(this)}>
        ...
        <span id={'score-'+this.props.word} name='score' className={styles.score + ' ' + (this.state.value==='flipped' ? styles.shown : styles.hidden)}> 
           <div id={'score-'+this.props.word+'-value'} className={styles.big}>{this.props.score}</div> 
           <div id={'score-'+this.props.word+'-word'} className={styles.small}>{this.props.word}</div>
        </span>
      </div>
    )
  }
